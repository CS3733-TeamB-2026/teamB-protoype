...
apps/
  backend/
    lib/                   Backend-only utilities (not shared via packages/db)
      ...
      extractors.ts        extractTextFromFile() — pulls plain text from PDF/DOCX/TXT for backfill
...
packages/db/
  ...
  lib/
    ...
    embeddings.ts          storeEmbedding() / queryEmbedding() — pgvector helpers
...
Embedding inputs are constructed by `buildEmbeddingInput(kind, item)` (`apps/backend/lib/embeddingInputs.ts`) — a plain-text representation of the entity's searchable fields. Text for file-based content is extracted by `extractTextFromFile()` (`apps/backend/lib/extractors.ts`) using pdf-parse and mammoth.
...
The `packages/db/lib/embeddings.ts` module provides the pgvector query helpers (`storeEmbedding`, `queryEmbedding`) used by each query class's `semanticSearch` method.
